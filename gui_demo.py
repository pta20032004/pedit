...
class VideoEditorMainWindow(QMainWindow):
    ...
    def setup_defaults(self):
        ...
        try:
            ...
            # 🔥 3. BANNER POSITION VÀ SIZE - GIÁ TRỊ TÙY CHỈNH CỦA BẠN
            if hasattr(self, 'banner_x'):
                self.banner_x.setValue(230)  # X position - GIÁ TRỊ CỦA BẠN
                self.add_log("INFO", f"📍 Default banner X position: 230px")
                
            if hasattr(self, 'banner_y'):
                self.banner_y.setValue(1400)  # Y position - GIÁ TRỊ CỦA BẠN
        ...
        if voice_data:
            for voice in voice_data:
                ...
                if "Nữ" in gender and "Nam" in gender:
                    gender_emoji = "🎭"  # Unisex voice
                elif "Nữ" in gender:
                    gender_emoji = "🟣"  # Female voice (purple circle)
                elif "Nam" in gender:
                    gender_emoji = "🔵"  # Male voice (blue circle)
                else:
